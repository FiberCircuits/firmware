...
uint8_t MC3672::readRegister8(uint8_t reg)
{
    ...
    Wire.requestFrom(deviceAddr, uint8_t(1)); //Once done, bus is released by default
    value = Wire.read();
    return value;
}
...
void MC3672::readRegisters(uint8_t reg, byte *buffer, uint8_t len)
{
    ...
    Wire.write(reg);
    Wire.endTransmission(false); //endTransmission but keep the connection active
    Wire.requestFrom(deviceAddr, len); //Ask for bytes, once done, bus is released by default
...
// set I2C device address: 0x4C (default) or 0x6C
void MC3672::setDeviceAddr(uint8_t addr)
{
    deviceAddr = addr;
}
...
bool MC3672::start(uint8_t addr)
{
     setDeviceAddr(addr);

     // TODO: allow changing the pins
     Wire.setSDA(PB7); // 14
     Wire.setSCL(PB6); // 13
     ...
     /* No MC3672 detected ... return false */
      ...
      SetRangeCtrl(MC3672_CFG_RANGE_DEFAULT);           //Range: 8G
      SetResolutionCtrl(MC3672_CFG_RESOLUTION_DEFAULT); //Resolution: 14bit
      SetCWakeSampleRate(MC3672_CFG_SAMPLE_RATE_CWAKE_DEFAULT); //Sampling Rate: 54Hz
      SetMode(MC3672_MODE_CWAKE); //Mode: Active
      // TODO: test if we need to set IPP_MODE_PUSH_PULL bit in the
      // MC3672_REG_INTR_C register as in the following example:
      // https://github.com/mcubemems/mCube_mc36xx_arduino_driver/blob/master/MC36XX.cpp#L426
      return true;
}
